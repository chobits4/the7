...
class The7_Admin_Dashboard_Settings {
	...
	public static function setup() {
		...
		add_action( 'the7_dashboard_before_settings_save', array( __CLASS__, 'maybe_erase_notices' ), 10, 2 );
	...
	/**
	 * Action. Remove notices
	 */
	public static function maybe_erase_notices( $new_settings, $old_settings ) {
		$silence_opt_name = 'silence-purchase-notification';
		if ( ! isset( $new_settings[ $silence_opt_name ], $old_settings[ $silence_opt_name ] ) ) {
			return;
		}
		if ( $new_settings[ $silence_opt_name ] !== $old_settings[ $silence_opt_name ] ) {
			the7_admin_notices()->reset( 'silence-purchase-notification' );
		}
	}
