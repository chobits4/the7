...
class GW_GoPricing_Admin {	
	...
	public static function ajax_action_router() {
		...
		/**		
		Removed in v3.3.12 - too much errors :)
		$error = error_get_last();
		if ( !empty( $error ) ) {
			GW_GoPricing_AdminNotices::add( 'ajax', 'error', sprintf( __( 'You have "%s" PHP error in "%s" file line %s!', 'go_pricing_textdomain' ) , $error['message'], $error['file'], $error['line']  ) );
