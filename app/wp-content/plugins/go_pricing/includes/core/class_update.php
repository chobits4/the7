...
class GW_GoPricing_Update {
	...
	public function update_info( $return, $action, $args ) {
		
		$update_data = get_option( self::$plugin_prefix . '_update_data', array() );

		if ( !empty( $args->slug ) ) $slug = $args->slug;
		
		if ( empty( $action ) || $action != 'plugin_information' || empty( $slug ) || empty( $update_data ) || empty( $update_data[$slug] ) || empty( $update_data[$slug]['api_data'] ) || empty( $update_data[$slug]['id'] ) ) return $return;
		...
		$obj->author = '<a href="https://granthweb.com" target="_blank">Granth</a>';
